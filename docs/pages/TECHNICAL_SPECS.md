...
- # Core Infrastructure: Universal Agent Hub (Deep Specification)
	...
	- ## MCP-Level Human Approval Gate System
		...
		- **State File:** `.squad-state.json` resolved dynamically at the root of the active project by traversing upward from `process.cwd()` looking for `.squad-state.json` or project markers (`.git`, `package.json`).
		- **Session Schema:**
			```json
			{
		...
		- **New MCP Tools (ref: `index.js`):**
			- **`pipeline_start`**: Initializes a new pipeline session. Accepts `{ goal: string, gates: string[] }`. Traverses upward from `process.cwd()` to resolve the active project root and creates/overwrites `.squad-state.json` with all gates set to `locked` and a fresh `session_id`. If `.gitignore` exists at the resolved project root, automatically appends `.squad-state.json` if it does not already contain it. Must be called before any other gate tool. Returns session confirmation and the resolved file path.
			- **`request_approval`**: Called by an LLM agent at the END of a phase. Accepts `{ gate: string, artifact_path?: string, summary: string }`. Resolves the dynamic state file path, sets the gate to `pending`, and returns a hard STOP message instructing the LLM to cease all further tool calls until the human approves. In standalone mode (no state file found), emits a soft STOP message without writing state.
			- **`check_gate`**: Called by an LLM agent at the START of the next phase. Accepts `{ gate: string }`. Resolves the dynamic state file path. Returns `{ approved: true }` if gate is `approved`. Returns a **hard error** (`isError: true`) if gate is `pending` or `locked` — the MCP error response prevents the calling LLM from treating this as a success and continuing. In standalone mode (no state file found), returns `{ approved: true, message: "No active Squad session — proceeding with prompt-level guardrails only." }` to allow standalone agent runs to proceed unblocked.
			- **`pipeline_approve`**: Called to register human approval for a gate. Accepts `{ gate: string }`. Updates `.squad-state.json`, transitioning the gate status to `approved` and stamping it with `approved_at`. Returns confirmation.
		- **Human Trust Anchor — `/squad:approve <gate>` / `pipeline_approve`:** The mechanisms to advance a gate from `pending` → `approved`. The slash command `/squad:approve` lives in `squad/commands/squad/approve.toml` and reads/writes the state file. The `pipeline_approve` tool is an MCP-level tool call. Both write `approved` status to `.squad-state.json`.
		- **Standalone Mode Grace:** When no `.squad-state.json` exists in the project traversal path, `check_gate` returns a soft advisory (`approved: true`) so that individual agents (architect, backend, frontend, mobile, automata, researcher, po) can run as standalone without being blocked by the gate system. Only `pipeline_start` activates the structural enforcement.
		- **Pipeline Gate Map:**
