...
- # Core Infrastructure: Universal Agent Hub (Deep Specification)
	...
	- ## MCP-Level Human Approval Gate System
		...
		- **Human Trust Anchor — `/squad:approve <gate>`:** The ONLY mechanism to advance a gate from `pending` → `approved`. Lives in `squad/commands/squad/approve.toml`. Reads and writes the updated state directly to the `.squad-state.json` file at the root of the active workspace. Cannot be triggered by an LLM tool call — requires explicit human command execution.
