...
- # Backend (Deep Specification)
	...
	- ## Commands
		...
		- **`backend:squad-review`**: Squad Flow peer review command (Phase 4.5). Reviews the committed branch using SonarQube MCP + test verification. On `APPROVE`: pushes to origin (`git push origin <branch>`). On `REQUEST CHANGES`: reports findings and halts; developer must fix and re-commit, then Phase 4.5 repeats. (ref: `backend/commands/backend/squad-review.toml`)
